...
When('I login with incorrect credentials', async function () {
    ...
    let loginButtonElement = await this.driver.$("#ember12");
...
When('I open the profile settings', async function () {
    let userSettingsButton = await this.driver.$("#ember48");
...
When('I fill the title {kraken-string} and description', async function (titlePost) {
    ...
    let descriptionTextArea = await this.driver.$('.koenig-editor__editor.__mobiledoc-editor');
  ...
  When('I close the page settings', async function () {
    let buttonSettings = await this.driver.$('.close.settings-menu-header-action');
    return await buttonSettings.click();
  });
...
When('I publish the page', async function () {
    let publishButton = await this.driver.$('.gh-publishmenu.ember-view');
    return await publishButton.click();
  });

When('I return to the dashboard', async function () {  
    await this.driver.refresh();
    return await this.driver.url('http://52.42.171.94:3001/ghost/#/site');
  });

  When('I update the page', async function () {
    await new Promise(r => setTimeout(r, 500));
    let updateButton = await this.driver.$('.ember-view.ember-basic-dropdown-trigger.gh-btn.gh-btn-outline.gh-publishmenu-trigger');
    await updateButton.click();
    await new Promise(r => setTimeout(r, 500));
    let confirmUpdateButton = await this.driver.$('.gh-btn.gh-btn-blue.gh-publishmenu-button.gh-btn-icon.ember-view');
    await confirmUpdateButton.click();

  });

  When('I return to the pages screens', async function () {
    let backButton = await this.driver.$('.f8.fw3');
...
When('I confirm the publish page', async function () {
    ...
    let continueButton = await this.driver.$('.gh-btn.gh-btn-blue.gh-publishmenu-button.gh-btn-icon.ember-view');
    await continueButton.click();
    
    await new Promise(r => setTimeout(r, 2500));
    let returnButton = await this.driver.$('.gh-btn.gh-btn-outline.gh-btn-link');    
...
When('I deleted the page', async function () {
    let deleteButton = await this.driver.$('.gh-btn.gh-btn-hover-red.gh-btn-icon.settings-menu-delete-button');
     ...
     await new Promise(r => setTimeout(r, 500));
    let confirmDeleteButton = await this.driver.$('.gh-btn.gh-btn-red.gh-btn-icon.ember-view');
...
Then('I open the page with name {kraken-string}', async function (pageName) {
    let pageCreated = await this.driver.$(".//*//ol[contains(@class, 'gh-list')]//*//h3[text() = '" + pageName + "']");
