...
context('Datos Aleatorios: Crear una Page con una palabra alfanumerica de 5 a 10 caracteres', () => {

    let title = faker.string.alphanumeric({ length: { min: 5, max: 10 } });
    let description = faker.string.alphanumeric({ length: { min: 5, max: 10 } });
    let urlSlug = faker.string.alphanumeric({ length: { min: 5, max: 10 } });

    beforeEach(() => {
       initGhost.visitGhost();
    });

    it('Creación de una palabra con titulo, descripción y slug con una palabra alfanumerica de 5 a 10 caracteres',() => {
        login.signinEvent();
